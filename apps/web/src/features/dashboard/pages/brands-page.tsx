...
import { BrandNotFoundError, useBrand, useUpsertBrand } from "#/features/dashboard/lib/brand-hooks";
...
function BrandEditorCard({
  ...
  function addAlias() {
    ...
    setValue("aliases", [...aliases, next], { shouldDirty: true, shouldValidate: true });
    setAliasInput("");
  }
  ...
  return (
    <Card className={`p-6 shadow-none ${mode === "create" ? "mx-auto max-w-2xl text-center" : ""}`}>
      ...
      <form className={`mt-6 space-y-4 text-left ${mode === "create" ? "max-w-xl mx-auto w-full" : ""}`} onSubmit={(event) => void onSubmit(event)}>
        <div className="space-y-2">
        ...
        <div className="space-y-2">
          ...
          {errors.domain ? <p className="text-xs text-destructive">{errors.domain.message}</p> : null}
        </div>
        ...
        <div className="space-y-2">
          ...
          <div className="flex gap-2">
            ...
            />
            <Button type="button" variant="outline" onClick={addAlias} disabled={!aliasInput.trim() || aliases.length >= 10}>
          ...
          <p className="text-xs text-muted-foreground">Up to 10 aliases. Use short names people mention in AI responses.</p>
          {errors.aliases ? <p className="text-xs text-destructive">{errors.aliases.message}</p> : null}
        </div>

        {saveError ? <Alert variant="error">{saveError}</Alert> : null}

        <div className="flex items-center gap-3">
          <Button type="submit" disabled={!isDirty || isSubmitting || isSaving} isLoading={isSubmitting || isSaving} loadingText="Saving…">
            Save
          </Button>
          {savedMessage ? (
            <span aria-live="polite" className="text-xs text-muted-foreground transition-opacity duration-300 opacity-100">
              Saved
            </span>
          ) : null}
        </div>
