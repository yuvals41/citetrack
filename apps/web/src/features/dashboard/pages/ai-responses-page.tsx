...
function ResponseCardSkeleton({ index }: { index: number }) {
  return (
    <Card className="rounded-xl border border-foreground/10 p-4" data-testid={`response-skeleton-${index}`}>
...
export function AIResponsesPage({ workspaceSlug = "default" }: AIResponsesPageProps) {
  ...
  const degraded = responsePayload && "degraded" in responsePayload ? responsePayload.degraded : null;
          ...
          {responses.isPending ? (
            <div data-testid="loading-skeletons" className="space-y-3">
              {Array.from({ length: 5 }).map((_, index) => (
                <ResponseCardSkeleton key={index} index={index} />
              ))}
            </div>
          ) : responses.error ? (
