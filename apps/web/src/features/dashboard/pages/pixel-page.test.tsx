...
function setQueries({
  ...
  const workspacesResult = workspaces === undefined
...
describe("PixelPage", () => {
  ...
  it("shows skeleton placeholders while loading workspaces", () => {
    ...
    const skeletons = document.querySelectorAll(".animate-pulse, [data-testid='skeleton'], .bg-muted\\/40");
    expect(skeletons.length >= 0).toBe(true);
    expect(screen.getByRole("banner")).toBeInTheDocument();
  });

  it("renders snippet, 4 KPI cards, and 2 tables when populated", () => {
    setQueries({ workspaces: [MOCK_WORKSPACE], snippet: "(function(){/* pixel */})();", stats: MOCK_STATS });
