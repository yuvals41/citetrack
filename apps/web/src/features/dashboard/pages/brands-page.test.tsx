    ...
    createCitetrackClient: () => ({
      ...
      getBrand: async () => {
        ...
        if (apiState.brandMode === "notFound") {
          throw new actual.ApiClientError(404, JSON.stringify({ detail: "Brand not found" }), "Brand not found");
        }
        if (!apiState.brand) {
...
describe("BrandsPage", () => {
  ...
  it("renders no workspace state", async () => {
    ...
    expect(screen.getByRole("link", { name: /complete onboarding/i })).toHaveAttribute("href", "/onboarding");
  });
