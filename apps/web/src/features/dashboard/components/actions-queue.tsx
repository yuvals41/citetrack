            ...
            {topActions.map((action, i) => (
              ...
              >
                <div className="flex flex-col gap-0.5 min-w-0">
                  <span className="text-sm font-medium truncate">{action.title ?? action.recommendation_code}</span>
                  {typeof action.description === "string" && action.description && (
                    <span className="text-xs text-muted-foreground line-clamp-2">{action.description}</span>
                  )}
                </div>
                {action.priority && (
                  <span
