...
import { ExternalLink, FileSearch, Network, Play, ShoppingBag, Sparkles } from "lucide-react";
...
function ResultNotice({
  degraded,
}: { degraded: { reason: string; message: string } | null | undefined }) {
  if (!degraded) {
    return null;
  }
...
function ScoreMiniCard({
  ...
  score,
  finding,
}: { title: string; score: number; finding: string }) {
  return (
  ...
  return (
    <div className="space-y-4" data-testid="extractability-result">
      ...
      <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-5">
        <ScoreMiniCard
          title="Summary block"
          score={result.summary_block.score}
          finding={result.summary_block.finding}
        />
        <ScoreMiniCard
          title="Section integrity"
          score={result.section_integrity.score}
          finding={result.section_integrity.finding}
        />
        <ScoreMiniCard
          title="Modular content"
          score={result.modular_content.score}
          finding={result.modular_content.finding}
        />
        <ScoreMiniCard
          title="Schema markup"
          score={result.schema_markup.score}
          finding={result.schema_markup.finding}
        />
        <ScoreMiniCard
          title="Static content"
          score={result.static_content.score}
          finding={result.static_content.finding}
        />
      </div>
        ...
        <div className="space-y-2">
          ...
          <div className="space-y-2">
            {result.recommendations.map((recommendation) => (
              <div
                key={recommendation}
                className="rounded-xl border border-foreground/10 px-3 py-2 text-sm ring-1 ring-foreground/10"
              >
                {recommendation}
              </div>
            ))}
    ...
    <div className="space-y-3" data-testid="crawler-sim-result">
      <div className="grid gap-2">
        {result.results.map((item) => (
          <div
            key={item.bot}
            className="grid gap-2 rounded-xl border border-foreground/10 p-3 text-sm ring-1 ring-foreground/10 md:grid-cols-[1.2fr_0.6fr_0.6fr_2fr]"
          >
    ...
    <div className="space-y-4" data-testid="entity-result">
      ...
      <div className="space-y-2">
        {rows.map(([label, present, url]) => (
          <div
            key={label}
            className="grid gap-2 rounded-xl border border-foreground/10 p-3 text-sm ring-1 ring-foreground/10 md:grid-cols-[1fr_0.7fr_2fr]"
          >
    ...
    <div className="space-y-4" data-testid="shopping-result">
      ...
      <div className="grid gap-2">
        {rows.map(([label, visible]) => (
          <div
            key={label}
            className="flex items-center justify-between rounded-xl border border-foreground/10 px-3 py-3 text-sm ring-1 ring-foreground/10"
          >
      ...
      <main className="flex-1 overflow-auto p-6">
        <div className="mx-auto max-w-6xl space-y-6">
          ...
          >
            <div className="grid gap-3 md:grid-cols-[1fr_auto]">
              ...
              <div className="flex items-end">
                <Button
                  ...
                  isLoading={extractability.isPending || crawlerSim.isPending}
                >
            ...
            {extractability.isPending && !extractability.data ? (
          ...
          >
            <div className="grid gap-3 md:grid-cols-2">
              <div className="space-y-2">
                ...
                <Input
                  ...
                  placeholder="best AI visibility tools for agencies"
                />
              </div>
              <div className="space-y-2">
            ...
            <div className="flex justify-end">
              <Button
                ...
                isLoading={queryFanout.isPending}
              >
                <Play className="size-4" />
                Run
              </Button>
              ...
              <div className="space-y-3" data-testid="query-fanout-result">
                <div className="rounded-xl border border-foreground/10 p-4 ring-1 ring-foreground/10">
                  ...
                  <p className="mt-2 text-3xl font-medium">
                    {Math.round(queryFanout.data.coverage * 100)}
                  </p>
                </div>
                <div className="space-y-2">
                  {queryFanout.data.results.map((item) => (
                    <div
                      key={item.sub_query}
                      className="grid gap-2 rounded-xl border border-foreground/10 p-3 text-sm ring-1 ring-foreground/10 md:grid-cols-[2fr_0.8fr_0.8fr]"
                    >
            ...
            <div className="grid gap-3 md:grid-cols-[1fr_auto]">
              <div className="space-y-2">
                ...
                <Input
                  ...
                  onChange={(event) => setEntityBrandName(event.target.value)}
                  placeholder="Citetrack"
                />
              </div>
              <div className="flex items-end">
                <Button
                  ...
                  isLoading={entityAnalysis.isPending}
                >
                  <Play className="size-4" />
                  Run
                </Button>
          ...
          >
            <div className="grid gap-3 md:grid-cols-[1fr_auto]">
              <div className="space-y-2">
                ...
                <Input
                  id="shopping-brand"
                  value={shoppingBrandName}
                  onChange={(event) => setShoppingBrandName(event.target.value)}
                  placeholder="Citetrack"
                />
              </div>
              <div className="flex items-end">
                <Button
                  ...
                  isLoading={shoppingAnalysis.isPending}
                >
                  <Play className="size-4" />
                  Run
                </Button>
