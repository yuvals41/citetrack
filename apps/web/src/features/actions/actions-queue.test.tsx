import type { OverviewSnapshot, TrendResponse } from "@citetrack/api-client";
...
function setDashboardQueries(
  actionsData:
    | {
        workspace: string;
        total_actions?: number;
        items?: never[];
        degraded?: { reason: string; message: string; recoverable: boolean };
      }
    | { degraded: { reason: string; message: string; recoverable: boolean } },
) {
  useQueryMock
...
describe("ActionsQueue", () => {
  ...
  it("renders populated actions", () => {
    ...
    expect(
      screen.getByText("Add evidence-backed answers for priority prompts."),
    ).toBeInTheDocument();
  ...
  it("renders the degraded notice on the dashboard", () => {
    ...
    expect(screen.getByText("fallback: Actions are temporarily unavailable")).toBeInTheDocument();
  });
});
